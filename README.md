...
- ✅ **Видеозвонки, демонстрация экрана и плитка тайлов** — камера и экран идут
  JPEG-кадрами по тому же DataChannel, что и голос (захват через `ffmpeg`). Раскладка
  «плитка»: своя камера, камера и экран собеседника. Захват (ffmpeg→MJPEG→кадры)
  проверен на реальном X-дисплее. Видео **не может нарушить аудио** — при любой
  ошибке (нет камеры/ffmpeg, Wayland без x11grab) звонок продолжается голосом.
  Работает **Linux ↔ Linux**; совместимость видео с текущим Windows-клиентом —
  отдельная доработка (там видео на WebRTC-медиа-треках), см. `docs/ROADMAP.md`.

## В работе (следующие этапы, код-«посадочные места» уже готовы)

- 🚧 **Групповые чаты** (плитка тайлов в звонке к этому уже готова)
...
Для **голосовых звонков** нужен **alsa-utils** (`arecord`/`aplay`), а для
**видео/демонстрации экрана** — **ffmpeg**:

```bash
sudo apt-get install -y alsa-utils ffmpeg      # Debian/Ubuntu
sudo pacman -S alsa-utils ffmpeg               # Arch/CachyOS
```
